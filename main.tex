...
\large
\begin{enumerate}
  \item \textbf{Aplicaciones del álgebra conmutativa y del álgebra real a sistemas de EDOs polinómicas} -- Alexandru Iosif \dotfill pag.\pageref{abs:1}

  \item \textbf{Polimerización, replicación y evolución del ARN en el origen de la vida} -- Carla Alejandre \dotfill pag.\pageref{abs:2}

  \item \textbf{Sistema dinámico para el vector PageRank con una memoria dependiente del tiempo} -- Daniel José Rodríguez Luis \dotfill pag.\pageref{abs:3}

  \item \textbf{Propagación no lineal de ultrasonidos en medios viscoelásticos con burbujas: hacia los metamateriales acústicos} -- Elena V. Carreras-Casanova \dotfill pag.\pageref{abs:4}

  \item \textbf{Conductividad térmica de una cadena de osciladores. El problema de Fermi-Pasta-Ulam-Tsingou} -- Fernán Del Río Martín \dotfill pag.\pageref{abs:5}

  \item \textbf{Intermitencia caótica en un láser de clase B con señal inyectada} -- Francisco Javier Marín Rodríguez \dotfill pag.\pageref{abs:6}

  \item \textbf{Clasificación de autómatas celulares bajo diferentes condiciones de contorno usando la distancia de Hamming} -- Gaspar Alfaro \dotfill pag.\pageref{abs:7}

  \item \textbf{Biritmicidad estocástica fuera de la región de coexistencia en el modelo de Hindmarsh-Rose} -- Ignacio Ortega Piwonka \dotfill pag.\pageref{abs:8}

  \item \textbf{Término de amortiguamiento fraccionario en osciladores no lineales} -- Mattia Coccolo \dotfill pag.\pageref{abs:9}

  \item \textbf{Diferenciación ontológica y navegación semántica en redes léxicas} -- Pablo García \dotfill pag.\pageref{abs:10}

  \item \textbf{Dinámica de solitones en generalizaciones complejas de la ecuación de mKdV} -- Paz Albares Vicente \dotfill pag.\pageref{abs:11}

  \item \textbf{Resiliencia inducida por dispersión a fluctuaciones ambientales estocásticas en poblaciones con efecto Allee} -- Rodrigo Crespo Miguel \dotfill pag.\pageref{abs:12}

  \item \textbf{Eficiencia termodinámica de estructuras disipativas acopladas} -- Álvaro García López \dotfill pag.\pageref{abs:13}
\end{enumerate}
...
\begin{tikzpicture}[remember picture,overlay]
    ...
    at ([xshift=-0.8cm,yshift=0.8cm]current page.south east)
    {\includegraphics[width=3.8cm]{assets/SINC_LOGO.png}};
\end{tikzpicture}

\vspace*{1.5cm}

\begin{center}
    {\Huge\bfseries Abstract #1}
\end{center}
...
\begin{center}
\includegraphics[width=0.5\textwidth]{figures/carla_alejandre_fig01_p1_crop.png}
...
{\textsuperscript{1} onlinear Dynamics, Chaos and Complex Systems Group, Departamento de Física, Universidad Rey Juan Carlos, Tulipán s/n, Móstoles, 28933, Madrid, Spain\\
...
{Pablo García}
{Complex Systems Group, Department of Applied Mathematics,
Universidad Polit´ecnica de Madrid, Avenida Juan de Herrera 6, 28040 Madrid, Spain.
2Complex Systems Group, Escuela T´ecnica Superior de Ingeniería Agronómica,
Alimentaria y de Biosistemas, Universidad Politécnica de Madrid,
Avenida Puerta de Hierro 2-4, 28040 Madrid, Spain.}
...
\begin{center}
\includegraphics[width=0.68\textwidth]{figures/alvaro_garcia_fig01_p1_crop.png}
